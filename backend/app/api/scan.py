...
def start_scan(request: ScanRequest | None = None) -> dict:
    if snapshot()["running"]: raise HTTPException(status_code=409, detail="A scan is already running")
    requested_path = Path(request.path) if request is not None and request.path else None
    try: root = validate_scan_root(requested_path)
